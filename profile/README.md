...
entertain-and-more is the entertainment branch of the `open-bricks` and `ellmos` ecosystem. Its public repositories focus on practical, inspectable local-first software: terminal chess with optional Claude API integration and tabletop role-playing support systems (rpx). Every public application is built to keep user data local while offering optional AI enhancement layers for players and Game Masters.

> [!NOTE]
> **Public Navigation Index:** Refreshed against live GitHub API metadata on **2026-07-29**. Every public repository visible in `entertain-and-more` (2 active software projects plus 1 profile repository) is indexed here. Private or internal work is intentionally omitted.
...
Checked **2026-07-29**: the public organization currently contains 2 software applications plus 1 profile repository.
